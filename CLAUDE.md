...
| **4e** | 🔴 **WHAT A MOB IS FOR — `enemy-scaling.md` S-6, PROPOSAL (owner gave three ideas, 2026-09-14).** ⭐ **They are not alternatives; they compose into one rule.** ① **THE PRESS** — *"mobs coordinate attacks to add force to each other, like a player's combined attack."* 🔴 **§5.7 ALREADY WRITES THIS RULE** — *"Combined attacks merge damage and count as ONE hit… the party's designed path to single-hit numbers no individual can reach"* — **and nothing restricts it to contestants.** The entire effect is one word: **resistance applies ONCE to the merged total** instead of once per mob. **No new number.** Curve is **identical on every floor** (both sides were calibrated together): **1 mob chaff · 2 hurts · 3 DESTROYS A TORSO** (F1 1/5/9 vs torso 7 · F9 8/27/46 vs torso 35). ⭐ **A mob becomes a POSITIONING threat**, answered by corridors (the §21.4 terrain I just wrote), not being surrounded, and §4's rooted-bulwark retarget. ⚠️ **It inverts the playtest tactic — spreading one-per-mob is what stops being safe.** ② **THE ELITE GOVERNS** — *"mobs are kind of useless on their own, and when governed by elite they gain buffs."* ⭐ **This is the governor ① needs** (12 mobs × 4 contestants = 3 each = a wipe) **and the best of the three**, because it fixes three things at once: it makes the playtest **correct rather than a bug** (leaderless mobs *should* be trivial); it **fills S-4's hole** — the ladder ran 28% → 83% with nothing between, and a governed horde IS the missing rung, without a second elite; and it writes a **target-priority decision** into every mixed room (kill the elite → the horde falls apart). ⚙️ It also finally justifies why an elite is standing in a room full of mobs. ③ **ARMOR DEGRADES** — *"over X attacks of Y strength, the resistance drops."* ⚠️ **Per-item hit counters are the wrong implementation**, but the effect is already on the sheet: **resistance on a part drops by the CONDITION TIER on that part.** ⭐ *"X attacks of Y strength"* is exactly what a tier already means — the armour is caved in because the body under it is — and it is the **precise mirror of §21.6 Body**, which already subtracts Force from what a conditioned limb *deals*. **Zero new tracking**, and composed with ① it is a death spiral with a **visible, curable** cause (§21.6 made healing a damage buff; this makes it a defence buff too). 🔴 **Total new machinery for all three: one condition on an existing rule.** ⚠️ Watch in play: ① kills a cornered contestant fast — that is the point and also the failure mode; the dial is ②'s reach (an elite directs what it can see, or a number, not the whole room) | `rulebook/enemy-scaling.md` **S-6** |

...
| **6** | ✅ **The weapon research sweep is COMPLETE (2026-08-25) — all 6 passes, 4 tranches, 25 myth weapons + 14 fiction/game systems, every entry from a page actually read.** 🔓 The Fandom block is bypassed: `/wiki/` page views are Cloudflare-403 but **`api.php` returns 200** (raw wikitext, higher fidelity than HTML). 🔴 **Three findings need an owner call, not more research:** (a) **W-7 §3** — the material band may already solve the grid's 9-floor axis, cutting the authoring target from 81 weapons to ~27 concepts; (b) **W-8 §1** — five sources gate apex items on **the item's own consent**, a requirement shape §12.1 has no version of; (c) **W-8 §5** — Frieren's Hero's Sword refused the actual hero and a **replica** won anyway, which for a show where the audience decides what is true is a native mechanic we already have the stats for. ⚠️ W-1 is **partly retracted** — its 'genre-wide ladder' was one wiki table duplicated, and every tranche-1 claim written from search summaries needed correcting once the page was read | `rulebook/weapon-research.md` |

...
| **6** | 🔬 **The weapon research sweep — 2 of 6 passes DONE (2026-08-25).** Egress resolved: `en.wikipedia.org` and `*.wiki.fextralife.com` are open; **`*.fandom.com` returns a Cloudflare 403, which no allowlist edit can fix**, so passes #2/#3/#4 need mirrors. **#1 classical myth** (25 weapons) gave the essence averages — myth gates on **requirement (64%)**, not cost (40%), and **88% of myth weapons are categorical with not one flat stat-bonus item**, which backs L-14 hard. **#6 game loot tables** gave the distribution shape — apex ratio ≈**2.9%**, and games are the near-inverse of myth on acquisition. 🔴 **Needs an owner call: W-7 §3** asks whether the material band already solves the grid's 9-floor axis, which would cut the authoring target from 81 weapons to ~27 concepts | `rulebook/weapon-research.md` |

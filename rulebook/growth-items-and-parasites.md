...
| ~~**4**~~ | ✅ **STRUCK 2026-09-22 — the bought half was ALREADY AUTHORED, and already correct.** All four store growth items — **`Mycelium Core`** (batch a) · **`Friendship Bracelet`** · **`Stray's Whistle`** · **`Prop Crown`** (batch c, C-4) — are `tier: 'Crude'`, which §19.3 prices at **1 UT**, *under* the Basic 3. ⭐ **So the constraint this row set was met before the row was written: price is not the tell.** 🔴 **But the SHELF was** — Odds & Ends held four items and all four were Growth. Closed the same day (2026-09-19) by **twelve curios** (`server/seeds/items-curios.js`, Misc now 16 lines / 25% Growth, with a gate in `shop-shelf.js` that exits 1 if Growth is ever the majority again) and by the owner's display ruling: **`Growth` is NEVER shown to a player** — `HIDDEN_SUBTYPES` + `publicSubtype()` in `constants.js` render it as its plain category, and one live leak in `character/LootBoxes.jsx` was fixed. ⚠️ **Still open and NOT struck:** §19.3 closes the store when the Lounge unlocks, so **the fantasy-item coupons still retire undistributed.** ~~The bought half — the store's growth stock, priced at or under Basic (3 UT) so price is not the tell — is not yet authored.~~ |
| ~~5~~ | ✅ **CLOSED 2026-09-22 — the fantasy item coupons were DISTRIBUTED** (owner). ~~(Basic weapon + one Lesser modifier each) are still undistributed, and they retire with the store.~~ |

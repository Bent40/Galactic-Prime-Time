...
> **↳ Re-pinned 2026-08-11.** An external review caught this document itself drifting: it
> stated the addendum runs R0–R25 when it now runs **R0–R29**. Corrected. The irony is the
> point — a drift detector needs its own check, so §4 now carries the exact commands. — book ↔ app ↔ addendum
...
That artifact was never written (`BOOK:v2/design/v1-v2-fork-spec.md:86-89`). Its
...
| **Digital rules (v2)** | `GAME:docs/rules-addendum.md` (**R0–R29**) | Engine rulings; the sim implements the addendum wherever the book is silent or broken | This repo |
...
**`BOOK:v2/design/v1-v2-fork-spec.md:39-50` — v1 is FROZEN; v2 bends.** Where the two
...
| **T-3** | **Followers / the exposure ladder** — *ruled 2026-08-10; not in any prior register* | **Abstract decaying count** of paying watchers at phone-vote money, billions-scale — `BOOK:...:880-884` | **Followers = reverence = divinity**: a roster of **named, individual living beings** (dozens, not millions). Viewers stay abstract billions — `BOOK:v2/design/v2-decisions.md:604`, `:627-631` (Q-18/Q-26) | v2 needed a divinity currency; the round-5 pass found Followers already were one. The round-2 doc itself calls this *"a mechanical fork, not a re-skin"* — `:615-618` | ✅ yes — but it **forks a shared mechanic**, so it must stay listed here |
| **T-4** | **Camera Call** — *broke convergence 2026-08-10* | Gains **AND** losses from the spotlit contestant are **doubled**; self-calls legal; one spotlight; ends with that contestant's current-or-next action — `BOOK:...:898-909` | **Double or nothing on a declared action** — the table gambles on the *specific act* the player names; a failed call **burns earned reverence** (named Followers walk away) — `BOOK:v2/design/v2-decisions.md:932` (D-10), `:1115-1120` (Q-32) | v2 needed the broadcast plane and the wager plane to fire together (Frame-C); a passive multiplier is variance, a declared bet is agency — `:943-953` | ✅ yes as a v2 fork — **but the addendum has not absorbed it: see U-4** |
...
| **U-7** | **Doc-state: D-26 was recorded as done before it existed** | *"Group D — housekeeping, applied: … D-26 (the three-way consistency guard, **written**)"* — `BOOK:v2/design/v2-decisions.md:997-998` | `docs/versions/three-way-consistency.md` did not exist when that line was written. A completion claim outran the artifact — exactly the failure mode this guard exists to catch | **None beyond this file existing** — the claim is now true. Logged so the pattern stays visible |
| **U-8** | **Both planning docs describe the addendum as "R0–R23"** | `GAME:docs/ttrpg-update-plan.md:18` and `BOOK:v2/design/v1-v2-fork-spec.md:78` both say *"`docs/rules-addendum.md` R0–R23"*. It actually runs **R0–R29** (26 `## Rn` headings; R24 at `:803`, R25 at `:826`) | Cosmetic, but it is a canon-inventory statement inside the two documents that *define* the canon inventory. R24 (feint-read) and R25 (Tactical Roll) are exactly the kind of shared-spine rulings this guard tracks | **Update both lines to R0–R29** when either doc is next touched |

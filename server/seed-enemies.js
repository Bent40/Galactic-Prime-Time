...
// ⭐ FLOOR 0 is the TUTORIAL (2026-09-15). forceAt() in floor-bands.js bakes in two
// §21.6 prep steps a tutorial party has not bought — no Forge, no coatings, no
// drilled assists, no band step — so a floor-0 contestant is weapon class 2 and
// nothing else: 2 Force. Hence mob 2 · elite 24 · boss 50 · super 120. ⭐ The
// Incinedile's Network is exactly 50, which is the boss centre for a floor below F1.
const FLOOR_MOB_HP = { 0: 2, 1: 5, 2: 6, 3: 7, 4: 8, 5: 9, 6: 10, 7: 11, 8: 12, 9: 13 };
...
const floor    = floorIdx !== -1 ? Number(process.argv[floorIdx + 1]) : 1;   // 0 is a real floor
...
function doctrineCheck(seeds, atFloor = floor) {
  ...
  if (!mobHp) throw new Error(`--floor ${atFloor} has no mob-HP entry (F0–F9 only)`);
  const problems = [];
  for (const e of seeds) {
...
function damageProblems(e, atFloor) {
  ...
  // ⚠️ `floor: 0` used to mean "unset, skip me". Floor 0 is the TUTORIAL now, so the
  // unset sentinel moved to absent/null/'' (models/Enemy.js matches). An un-migrated
  // entry still skips; a tutorial entry is gated like any other.
  if (!sig || sig.floor === undefined || sig.floor === null || sig.floor === '') return [];
  const out = [];
  const band = FLOOR_DAMAGE[Number(sig.floor)];
  if (!band) return [`${e.name}: signature.floor ${sig.floor} is not F0–F9`];

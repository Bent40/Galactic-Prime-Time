...
router.get('/skills', requireAuth, async (req, res) => {
  try {
    const templates = await SkillTemplate.find(
      {},
      'name momentCost stats passive capacity maxCapacity requirements range target effect description keywords levelEffects origin animalOnly raceLock exclusiveTo damageTypes'
    ).sort({ name: 1 }).lean();
    res.json(templates);
  } catch (err) {
    res.status(500).json({ error: 'Server error' });
